...
from wand.image import *  # 이미지 처리 기능 제공하는 이미지 매직의 라이브러리


# 함수 선언 부분
def displayImage(img, width, height):  # 이미지 화면에 출력하는 함수
    ...
    # 이미지 폭, 높이만큼 반복해서 픽셀의 RGB 값 추출 후 paper에 컬러로 점 찍기 - 결국 이미지 크기만큼 화면에 출력
...
def func_open():
    ...
    readFp = askopenfilename(parent=window, filetypes=(("모든 그림 파일", "*.jpg; *.jpeg; *.bmp; *.png; *.tif; *.gif"),
                                                      ("모든 파일", "*.*")))
    photo = Image(filename=readFp)  # Image : Wand 라이브러리에서 제공(=PhotoImage)
    oriX = photo.width
    oriY = photo.height
    photo2 = photo.clone()
    newX = photo2.width
    newY = photo2.height
    displayImage(photo2, newX, newY)  # displayImage(이미지, 폭, 높이) 함수 실행


def func_save():
    global window, canvas, paper, photo, photo2, oriX, oriY

    if photo2 == None:  # 결과 이미지가 없으면 저장할 것이 없으므로 그냥 빠져나감
        return
    saveFp = asksaveasfile(parent=window, mode="w", defaultextension=".jpg",
                           filetypes=(("jpg 파일", "*.jpg; *.jpeg"), ("모든 파일", "*.*")))  # 저장할 파일 입력 받기
    savePhoto = photo2.convert("jpg")  # 결과 이미지 JPG 파일로 변환
    savePhoto.save(filename=saveFp.name)  # 변환한 JPG 파일 저장(saveFp 결과의 name 저장)
...
def func_zoomin():
    ...
    photo2 = photo.clone()
    photo2.resize(int(oriX * scale), int(oriY * scale))  # resize(500, 500)이면 500x500 크기로 변경
    newX = photo2.width
    newY = photo2.height
...
def func_bright():
    ...
    photo2 = photo.clone()
    photo2.modulate(value, 100, 100)  # modulate(밝기 값, 100, 100), 100이면 원본 밝기
    newX = photo2.width
    newY = photo2.height
    displayImage(photo2, newX, newY)
...
def func_clear():
    ...
    photo2 = photo.clone()
    photo2.modulate(100, value, 100)  # modulate(100, 선명도 값, 100), 100이면 원본 선명도
    newX = photo2.width
    newY = photo2.height
    displayImage(photo2, newX, newY)
...
def func_bw():
    global window, canvas, paper, photo, photo2, oriX, oriY
    photo2 = photo.clone()
    photo2.type = "grayscale"  # 컬러 이미지를 흑백 이미지로 변경하기
    newX = photo2.width
    newY = photo2.height
    displayImage(photo2, newX, newY)
...
photo, photo2 = None, None  # photo 원본 이미지, photo2 이미지 처리 결과
oriX, oriY = 0, 0  # 원본 이미지 폭, 높이 저장
